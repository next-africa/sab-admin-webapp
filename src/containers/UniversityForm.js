...
class UniversityForm extends Component {
  constructor(props) {
    ...
    this.handleUniversityLinkChange = this.handleUniversityLinkChange.bind(
      this
    );
    ...
    this.handleProgramListLink = this.handleProgramListLink.bind(this);
    this.handleUniversityNameChange = this.handleUniversityNameChange.bind(
      this
    );
  ...
  handleUniversityLinkChange(e) {
    this.setState({ website: e.target.value }, () =>
      console.log("website link :", this.state.website)
    );
  }
  ...
  handleLangSelection(e) {
    ...
    if (this.state.selectedLanguages.indexOf(newSelection) > -1) {
      newSelectionArray = this.state.selectedLanguages.filter(
        s => s !== newSelection
      );
    } else {
      newSelectionArray = [...this.state.selectedLanguages, newSelection];
    }
    this.setState({ selectedLanguages: newSelectionArray }, () =>
      console.log("universite Language:", this.state.selectedLanguages)
    );
  }
  ...
  handleProgramListLink(e) {
    this.setState({ programListLink: e.target.value }, () =>
      console.log("Program list link :", this.state.programListLink)
    );
  }
  handleUniversityNameChange(e) {
    this.setState({ name: e.target.value }, () =>
      console.log("University name:", this.state.name)
    );
  }
  handleAddressCityChange(e) {
    this.setState({ addressCity: e.target.value }, () =>
      console.log("City:", this.state.addressCity)
    );
  }
  handleAddressLineChange(e) {
    this.setState({ addressLine: e.target.value }, () =>
      console.log("Line:", this.state.addressLine)
    );
  }
  handleAddressStateChange(e) {
    this.setState({ addressState: e.target.value }, () =>
      console.log("state:", this.state.addressState)
    );
  }
  handleAddressCodeChange(e) {
    this.setState({ addressCode: e.target.value }, () =>
      console.log("Code:", this.state.addressCode)
    );
  }
  handleTuitionAmountChange(e) {
    this.setState({ tuitionAmount: parseInt(e.target.value) }, () =>
      console.log("tuitionAmount:", this.state.tuitionAmount)
    );
  }

  handleTuitionLinkChange(e) {
    this.setState({ tuitionLink: e.target.value }, () =>
      console.log("tuitionAmount:", this.state.tuitionLink)
    );
  }
